...
def main() -> None:
    ...
    parser.add_argument("--queries-per-template", "-n", type=int, required=True,
                        help="Number of queries to generate per template.")
    parser.add_argument("--out-dir", "-o", type=str, default=".", help="Directory to write the generated queries to.")
    parser.add_argument("--template-pattern", "-p", type=str, default="*.toml",
    ...
    for template_file in pathlib.Path(args.template_dir).glob(args.template_pattern):
        ...
        queries = ceb.generate_workload(args.template_dir, queries_per_template=args.queries_per_template,
                                        template_pattern=local_glob, db_connection=pg_instance)
