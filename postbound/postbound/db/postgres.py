...
from postbound.qal import base, clauses, expressions, formatter, qal, transform
from postbound.util import collections as collection_utils
from postbound.util import dicts as dict_utils
from postbound.util import errors, logging
from postbound.util import misc as utils
from postbound.util import system as sys_util
...
class PostgresConfiguration(str):
    ...
    def load(**kwargs) -> PostgresConfiguration:
        ...
        return PostgresConfiguration(
            [PostgresSetting(key, val) for key, val in kwargs.items()]
        )
...
class PostgresInterface(db.Database):
    ...
    def prewarm_tables(
        ...
        *more_tables : base.TableReference
        ...
        tables: Iterable[base.TableReference] = list(
            collection_utils.enlist(tables)
        ) + list(more_tables)
        if not tables:
            return
        tables = set(
        ...
        indexes_to_prewarm = {
            idx
            for idx, primary in collection_utils.flatten(table_indexes)
    ...
    def _obtain_geqo_state(self) -> _GeQOState:
        ...
        for name, value in self._cursor.fetchall():
            if name == "geqo":
                ...
                raise RuntimeError(
                    "Malformed GeQO query. This is a programming error, it's not your fault!"
                )
    ...
    def _fetch_index_relnames(
        self, table: base.TableReference | str
    ...
    def _assert_active_extension(
        ...
        if is_shared_object or extension_name in ("pg_hint_plan", "pg_lab"):
            shared_object_name = (
                f"{extension_name}.so"
                if not extension_name.endswith(".so")
                else extension_name
            )
            loaded_shared_objects = sys_util.open_files(
                self._connection.info.backend_pid
            )
            extension_is_active = any(
                so.endswith(shared_object_name) for so in loaded_shared_objects
            )
        else:
            self._cursor.execute("SELECT extname FROM pg_extension;")
            extension_is_active = any(
                ext[0] == extension_name for ext in self._cursor.fetchall()
            )
...
class PostgresSchemaInterface(db.DatabaseSchema):
    ...
    def lookup_column(
        ...
        candidate_tables: list[base.TableReference],
    ) -> base.TableReference:
    ...
    def datatype(self, column: base.ColumnReference) -> str:
        ...
        query_template = textwrap.dedent(
            ...
            WHERE table_name = '{tab}' AND column_name = '{col}'""".format(
                tab=column.table.full_name, col=column.name
            )
        )
...
class PostgresStatisticsInterface(db.DatabaseStatistics):
    ...
    def update_statistics(
        self,
        columns: Optional[base.ColumnReference | Iterable[base.ColumnReference]] = None,
        *,
        tables: Optional[base.TableReference | Iterable[base.TableReference]] = None,
        ...
        if not columns and tables:
            ...
            columns = collection_utils.set_union(
                self._db.schema().columns(tab) for tab in tables
            )

        assert columns is not None
        columns: Iterable[base.ColumnReference] = collection_utils.enlist(columns)
        columns_map: dict[base.TableReference, list[str]] = dict_utils.generate_multi(
    ...
    def _retrieve_total_rows_from_stats(
        self, table: base.TableReference
    ) -> Optional[int]:
        count_query = (
    ...
    def _retrieve_distinct_values_from_stats(
        self, column: base.ColumnReference
    ) -> Optional[int]:
        dist_query = (
    ...
    def _retrieve_min_max_values_from_stats(
        ...
        if not self.enable_emulation_fallback:
            raise db.UnsupportedDatabaseFeatureError(
                self._db, "min/max value statistics"
            )
        return self._calculate_min_max_values(column, cache_enabled=True)

    def _retrieve_most_common_values_from_stats(
        self, column: base.ColumnReference, k: int
...
class HintParts:
    ...
    def merge_with(self, other: HintParts) -> HintParts:
        ...
        merged_hints = self.hints + [
            hint for hint in other.hints if hint not in self.hints
        ]
...
def _is_hash_join(
    ...
    if operator_assignment is not None:
        selected_operator: Optional[physops.JoinOperatorAssignment] = (
            operator_assignment[join_tree_node.tables()]
        )
        ...
        global_join_ops = {
            op
            for op in operator_assignment.get_globally_enabled_operators()
            if isinstance(op, physops.JoinOperators)
        }
        return (
            len(global_join_ops) == 1
            and physops.JoinOperators.HashJoin in global_join_ops
        )
    ...
    return (
        join_tree_node.annotation.operator is not None
        and join_tree_node.annotation.operator.operator
        == physops.JoinOperators.HashJoin
    )
...
def _apply_hint_block_to_query(
    query: qal.SqlQuery, hint_block: Optional[clauses.Hint]
) -> qal.SqlQuery:
...
PostgresScanHints = {
    ...
    physops.ScanOperators.BitmapScan,
}
"""All scan operators that are supported by Postgres."""

PostgresPlanHints = {
    planparams.HintType.CardinalityHint,
    planparams.HintType.ParallelizationHint,
    planparams.HintType.JoinOrderHint,
    planparams.HintType.JoinSubqueryHint,
    planparams.HintType.JoinDirectionHint,
    planparams.HintType.OperatorHint,
}
...
def _replace_postgres_cast_expressions(
    ...
    """
    return (
        _PostgresCastExpression(expression)
        if isinstance(expression, expressions.CastExpression)
        else expression
    )
...
class PostgresHintService(db.HintService):
    ...
    def generate_hints(
        ...
        query: qal.SqlQuery,
        join_order: Optional[
            jointree.LogicalJoinTree | jointree.PhysicalQueryPlan
        ] = None,
        physical_operators: Optional[physops.PhysicalOperatorAssignment] = None,
        plan_parameters: Optional[planparams.PlanParameterization] = None,
    ) -> qal.SqlQuery:
        ...
        physical_operators = (
            join_order.physical_operators()
            if not physical_operators
            and isinstance(join_order, jointree.PhysicalQueryPlan)
            else physical_operators
        )
        plan_parameters = (
            join_order.plan_parameters()
    ...
    def format_query(self, query: qal.SqlQuery) -> str:
        ...
        if query.limit_clause and not isinstance(
            query.limit_clause, PostgresLimitClause
        ):
            query = transform.replace_clause(
                query, PostgresLimitClause(query.limit_clause)
            )
        return formatter.format_quick(query)

    def supports_hint(
        self, hint: physops.PhysicalOperator | planparams.HintType
    ) -> bool:
        self._assert_active_backend()
    ...
    def _generate_leading_hint_content(
        self,
        ...
        has_directional_information = isinstance(
            join_tree_node.annotation, jointree.PhysicalJoinMetadata
        ) and isinstance(
            join_tree_node.annotation.operator,
            physops.DirectionalJoinOperatorAssignment,
        )
        if has_directional_information:
            annotation: physops.DirectionalJoinOperatorAssignment = (
                join_tree_node.annotation.operator
            )
            inner_tables = annotation.inner
            inner_child = (
                join_tree_node.left_child
                if join_tree_node.left_child.tables() == inner_tables
                else join_tree_node.right_child
            )
            outer_child = (
                join_tree_node.left_child
                if inner_child == join_tree_node.right_child
                else join_tree_node.right_child
            )
            inner_child, outer_child = (
                (outer_child, inner_child)
                if annotation.operator == physops.JoinOperators.HashJoin
        ...
        inner_hint = self._generate_leading_hint_content(
            inner_child, operator_assignment
        )
        outer_hint = self._generate_leading_hint_content(
            outer_child, operator_assignment
        )
    ...
    def _generate_pg_join_order_hint(
        ...
        if len(join_order) < 2:
            return query, None
        leading_hint = self._generate_leading_hint_content(
            join_order.root, operator_assignment
        )
        leading_hint = (
    ...
    def _generate_pg_operator_hints(
        self, physical_operators: physops.PhysicalOperatorAssignment
    ) -> HintParts:
    ...
    def _generate_pg_parameter_hints(
        self, plan_parameters: planparams.PlanParameterization
    ) -> HintParts:
...
class PostgresOptimizer(db.OptimizerInterface):
    ...
    def analyze_plan(self, query: qal.SqlQuery) -> db.QueryExecutionPlan:
        self._pg_instance._current_geqo_state = self._pg_instance._obtain_geqo_state()
        query, deactivated_geqo = self._pg_instance._prepare_query_execution(
            transform.as_explain_analyze(query)
        )
...
def connect(
    *,
    ...
    postgres_db = PostgresInterface(
        connect_string, system_name=name, cache_enabled=cache_enabled, debug=debug
    )
    if not private:
        orig_name = name
        instance_idx = 2
...
def _parallel_query_worker(
    ...
    log(
        f"[worker id={threading.get_ident()}, ts={logging.timestamp()}] Now executing query {query}"
    )
    try:
        cursor.execute(str(query))
        log(
            f"[worker id={threading.get_ident()}, ts={logging.timestamp()}] Executed query {query}"
        )
    except psycopg.errors.QueryCanceled as e:
        if "canceling statement due to statement timeout" in e.args:
            log(
                f"[worker id={threading.get_ident()}, ts={logging.timestamp()}] Query {query} timed out"
            )
            return query, None
        else:
            raise e
...
def _timeout_query_worker(
    ...
    error_sender: mp_conn.Connection,
) -> None:
    """Internal function to the `TimeoutQueryExecutor` to run individual queries.

    Query results are sent via the pipe, not as a return value. In case of an error, the error is not send via the
    result pipe, but rather via a separate error pipe.
    ...
    error_sender : mp_conn.Connection
        Pipe connection to send any errors that occur during the query execution
    """
    try:
        result = pg_instance.execute_query(query)
        query_result_sender.send(result)
    except (db.DatabaseServerError, db.DatabaseUserError) as e:
        error_sender.send(e)
...
class TimeoutQueryExecutor:
    ...
    def execute_query(self, query: qal.SqlQuery | str, timeout: float) -> Any:
        ...
        error_receiver, error_sender = mp.Pipe(False)
        query_execution_worker = mp.Process(
            target=_timeout_query_worker,
            args=(query, self._pg_instance, query_result_sender, error_sender),
        )
...
PostgresExplainJoinNodes = {
    "Nested Loop": physops.JoinOperators.NestedLoopJoin,
...
PostgresExplainScanNodes = {
    "Seq Scan": physops.ScanOperators.SequentialScan,
...
class PostgresExplainNode:
    ...
    def is_analyze(self) -> bool:
        ...
        """
        return not math.isnan(self.execution_time) or not math.isnan(
    ...
    def as_query_execution_plan(self) -> db.QueryExecutionPlan:
        ...
        for child in self.children:
            ...
            found_inner = (parent_rel == "Inner" and not found_hash_join) or (
                parent_rel == "Outer" and found_hash_join
            )
            found_outer = (parent_rel == "Outer" and not found_hash_join) or (
                parent_rel == "Inner" and found_hash_join
            )
            if found_inner:
        ...
        )  # in Postgres the control worker also processes input
        ...
        return db.QueryExecutionPlan(
            ...
            cost=self.cost,
            estimated_cardinality=self.cardinality_estimate,
            true_cardinality=true_card,
            execution_time=self.execution_time,
            cached_pages=self.shared_blocks_cached,
            scanned_pages=self.shared_blocks_read,
            physical_operator=operator,
            inner_child=inner_child,
            subplan_input=subplan_child,
    ...
    def inspect(self, *, _indentation: int = 0) -> str:
        ...
        child_inspections = [
            child.inspect(_indentation=_indentation + 2)
            for child in self.inner_outer_children()
        ]
...
class WorkloadShifter:
    ...
    def remove_random(
        ...
        *,
        ...
        """
        table_name = (
            table.full_name if isinstance(table, base.TableReference) else table
        )
    ...
    def generate_marker_table(
        ...
        *,
        ...
        target_col_ref = base.ColumnReference(
            target_column, base.TableReference(target_table)
        )
    ...
    def _determine_row_cnt(
        ...
        total_n_rows = self.pg_instance.statistics().total_rows(
            base.TableReference(table), cache_enabled=False, emulated=True
        )
